...
class TripletLoss(nn.Module):
    def __init__(self, margin):
        ...
        self.ssim = SSIM(data_range=1, channel=3)
        self.margin = margin

    def forward(self, pred, src, ref, mask):
        ssim_pred_src = 1. - self.ssim(pred * mask, src * mask)
        ssim_pred_ref = 1. - self.ssim(pred * mask, ref * mask)
        
        return F.relu(ssim_pred_src + self.margin - ssim_pred_ref).sum(), ssim_pred_src, ssim_pred_ref
